...
Class Master extends DBConnection {
	...
	function place_order(){
		...
		// Format traveller details as a JSON string to store in the orders table
		$traveller_details = [];
		for ($i = 0; $i < count($_POST['traveller_name']); $i++) {
			$traveller_details[] = [
				'name' => $_POST['traveller_name'][$i],
				'gender' => $_POST['gender'][$i],
				'date_of_birth' => $_POST['date_of_birth'][$i]
			];
		}
		$traveller_json = json_encode($traveller_details); // Convert to JSON format for storage
	
		// Prepare order data including traveller details
		$data = " client_id = '{$client_id}' ";
		$data .= " ,payment_method = '{$payment_method}' ";
		$data .= " ,amount = '{$amount}' ";
		$data .= " ,paid = '{$paid}' ";
		...
		$data .= " ,traveller_details = '{$traveller_json}' "; // Store JSON data
		$order_sql = "INSERT INTO `orders` SET $data";
		...
		if ($save_order) {
			$order_id = $this->conn->insert_id;
			$data = '';
			$cart = $this->conn->query("SELECT c.*, p.product_name, i.size, i.price, p.id as pid, i.unit, c.quantity 
										FROM `cart` c 
	...
	function feedback_user(){
		...
		$check = $this->conn->query("SELECT * FROM `tbl_feedback` where `email` = '{$email}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			$sql = "INSERT INTO `tbl_feedback` set {$data} ";
			$save = $this->conn->query($sql);
			$id = $this->conn->insert_id;
		}else{
			$sql = "UPDATE `tbl_feedback` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
